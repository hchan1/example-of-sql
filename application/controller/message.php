...
//       $message = $this->message_model->getAllMessage();


    $entries = $this->messages->getAllMessage();
        $number_of_entries = $this->messages->getAmountOfMessage();
 ...
 public function search1()
{
	...
	//if(isset($_GET["Search2"]))
	{
		$name="name";
		//session_start();
		//$_SESSION["name"]=$_POST["Search2"];
		bcookie($name,$_POST["Search2"]);
	}
	header('location: '. URL . 'songs/index');
}
public function addmessage()
{
	$rest3 =  $this->messages->getAmountOfMessage() ;
	require APP. 'view/_templates/header.php';
	$sender = $_COOKIE["sender"];
	$reciever = $_COOKIE["Reciever"];
        // if we have POST data to create a new song entry
        if (isset($_POST["submit_add_message"])) 
	{
	$this->messages->addmessage($rest3,$sender,$reciever,$_POST['Description']);
	echo $rest3. " " .$sender." ".$reciever." ".$_POST['Description'];
        }

        // where to go after song has been added
    	 header('location: ' . URL . 'message?RECIEVER='.$reciever);
}
